...
class TestMappedBase extends StatefulWidget {
  static const routeName = '/test/MappedBase';

  const TestMappedBase({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestMappedBaseState();
  }
}

class _TestMappedBaseState extends State<TestMappedBase> {
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'MappedBase', bizId);
      setState(() {
        _bean = bean;
      });
    }
