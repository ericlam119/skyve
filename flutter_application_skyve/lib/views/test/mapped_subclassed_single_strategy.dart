...
class TestMappedSubclassedSingleStrategy extends StatefulWidget {
  static const routeName = '/test/MappedSubclassedSingleStrategy';

  const TestMappedSubclassedSingleStrategy({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestMappedSubclassedSingleStrategyState();
  }
}

class _TestMappedSubclassedSingleStrategyState extends State<TestMappedSubclassedSingleStrategy> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestMappedSubclassedSingleStrategy.routeName,
        _bean['_title'],
        Visibility(
      ...
      SkyveVBox(children: [
        SkyveBorder(children: [
          SkyveForm(children: [
            ...
            BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
              BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Dynamic Subclass Integer')),
              BootstrapCol(sizes: 'col-12 col-md-8', child: 
                TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Dynamic Subclass Integer'), initialValue: nvl(_bean['dynamicSubclassInteger'])),
              ),
            ]),
          ]),
        ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'MappedSubclassedSingleStrategy', bizId);
      setState(() {
        _bean = bean;
      });
    }
