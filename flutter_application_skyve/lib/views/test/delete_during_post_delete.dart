...
class TestDeleteDuringPostDelete extends StatefulWidget {
  static const routeName = '/test/DeleteDuringPostDelete';

  const TestDeleteDuringPostDelete({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestDeleteDuringPostDeleteState();
  }
}

class _TestDeleteDuringPostDeleteState extends State<TestDeleteDuringPostDelete> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestDeleteDuringPostDelete.routeName,
        _bean['_title'],
        Visibility(
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'DeleteDuringPostDelete', bizId);
      setState(() {
        _bean = bean;
      });
    }
