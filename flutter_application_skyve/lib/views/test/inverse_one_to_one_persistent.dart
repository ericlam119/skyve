...
class TestInverseOneToOnePersistent extends StatefulWidget {
  static const routeName = '/test/InverseOneToOnePersistent';

  const TestInverseOneToOnePersistent({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestInverseOneToOnePersistentState();
  }
}

class _TestInverseOneToOnePersistentState extends State<TestInverseOneToOnePersistent> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestInverseOneToOnePersistent.routeName,
        _bean['_title'],
        Visibility(
    ...
    SkyveForm(children: [
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        ...
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('LookupDescription ${_bean["aggAssociation"]}'),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Inverse')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('LookupDescription ${_bean["invAggAssociation"]}'),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'InverseOneToOnePersistent', bizId);
      setState(() {
        _bean = bean;
      });
    }
