...
class TestDynamicMappedExtension extends StatefulWidget {
  static const routeName = '/test/DynamicMappedExtension';

  const TestDynamicMappedExtension({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestDynamicMappedExtensionState();
  }
}

class _TestDynamicMappedExtensionState extends State<TestDynamicMappedExtension> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestDynamicMappedExtension.routeName,
        _bean['_title'],
        Visibility(
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'DynamicMappedExtension', bizId);
      setState(() {
        _bean = bean;
      });
    }
