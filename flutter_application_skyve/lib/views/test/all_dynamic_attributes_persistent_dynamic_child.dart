...
class TestAllDynamicAttributesPersistentDynamicChild extends StatefulWidget {
  static const routeName = '/test/AllDynamicAttributesPersistentDynamicChild';

  const TestAllDynamicAttributesPersistentDynamicChild({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestAllDynamicAttributesPersistentDynamicChildState();
  }
}

class _TestAllDynamicAttributesPersistentDynamicChildState extends State<TestAllDynamicAttributesPersistentDynamicChild> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestAllDynamicAttributesPersistentDynamicChild.routeName,
        _bean['_title'],
        Visibility(
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'AllDynamicAttributesPersistentDynamicChild', bizId);
      setState(() {
        _bean = bean;
      });
    }
