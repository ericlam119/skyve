...
class TestUniqueConstraintOptimisation extends StatefulWidget {
  static const routeName = '/test/UniqueConstraintOptimisation';

  const TestUniqueConstraintOptimisation({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestUniqueConstraintOptimisationState();
  }
}

class _TestUniqueConstraintOptimisationState extends State<TestUniqueConstraintOptimisation> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestUniqueConstraintOptimisation.routeName,
        _bean['_title'],
        Visibility(
    ...
    SkyveForm(children: [
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        ...
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Text'), initialValue: nvl(_bean['uc1'])),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Text')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Text'), initialValue: nvl(_bean['uc2'])),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Text')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Text'), initialValue: nvl(_bean['uc3'])),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Non Persistent Association')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('LookupDescription ${_bean["nonPersistentAssociation"]}'),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Persistent Association')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('LookupDescription ${_bean["persistentAssociation"]}'),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'UniqueConstraintOptimisation', bizId);
      setState(() {
        _bean = bean;
      });
    }
