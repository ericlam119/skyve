...
class TestAnyDerived1 extends StatefulWidget {
  static const routeName = '/test/AnyDerived1';

  const TestAnyDerived1({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestAnyDerived1State();
  }
}

class _TestAnyDerived1State extends State<TestAnyDerived1> {
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      ...
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Text 1')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Text 1'), initialValue: nvl(_bean['text1'])),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'AnyDerived1', bizId);
      setState(() {
        _bean = bean;
      });
    }
