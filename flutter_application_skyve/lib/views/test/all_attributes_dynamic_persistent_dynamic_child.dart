...
class TestAllAttributesDynamicPersistentDynamicChild extends StatefulWidget {
  static const routeName = '/test/AllAttributesDynamicPersistentDynamicChild';

  const TestAllAttributesDynamicPersistentDynamicChild({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestAllAttributesDynamicPersistentDynamicChildState();
  }
}

class _TestAllAttributesDynamicPersistentDynamicChildState extends State<TestAllAttributesDynamicPersistentDynamicChild> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestAllAttributesDynamicPersistentDynamicChild.routeName,
        _bean['_title'],
        Visibility(
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'AllAttributesDynamicPersistentDynamicChild', bizId);
      setState(() {
        _bean = bean;
      });
    }
