...
class TestInjectedDocument extends StatefulWidget {
  static const routeName = '/test/InjectedDocument';

  const TestInjectedDocument({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestInjectedDocumentState();
  }
}

class _TestInjectedDocumentState extends State<TestInjectedDocument> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestInjectedDocument.routeName,
        _bean['_title'],
        Visibility(
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'InjectedDocument', bizId);
      setState(() {
        _bean = bean;
      });
    }
