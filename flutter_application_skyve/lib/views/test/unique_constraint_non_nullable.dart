...
class TestUniqueConstraintNonNullable extends StatefulWidget {
  static const routeName = '/test/UniqueConstraintNonNullable';

  const TestUniqueConstraintNonNullable({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestUniqueConstraintNonNullableState();
  }
}

class _TestUniqueConstraintNonNullableState extends State<TestUniqueConstraintNonNullable> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestUniqueConstraintNonNullable.routeName,
        _bean['_title'],
        Visibility(
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'UniqueConstraintNonNullable', bizId);
      setState(() {
        _bean = bean;
      });
    }
