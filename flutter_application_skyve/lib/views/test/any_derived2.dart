...
class TestAnyDerived2 extends StatefulWidget {
  static const routeName = '/test/AnyDerived2';

  const TestAnyDerived2({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestAnyDerived2State();
  }
}

class _TestAnyDerived2State extends State<TestAnyDerived2> {
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      ...
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Text 2')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Text 2'), initialValue: nvl(_bean['text2'])),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'AnyDerived2', bizId);
      setState(() {
        _bean = bean;
      });
    }
