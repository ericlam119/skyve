...
class TestHierarchical extends StatefulWidget {
  static const routeName = '/test/Hierarchical';

  const TestHierarchical({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestHierarchicalState();
  }
}

class _TestHierarchicalState extends State<TestHierarchical> {
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'Hierarchical', bizId);
      setState(() {
        _bean = bean;
      });
    }
