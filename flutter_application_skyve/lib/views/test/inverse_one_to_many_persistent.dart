...
class TestInverseOneToManyPersistent extends StatefulWidget {
  static const routeName = '/test/InverseOneToManyPersistent';

  const TestInverseOneToManyPersistent({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _TestInverseOneToManyPersistentState();
  }
}

class _TestInverseOneToManyPersistentState extends State<TestInverseOneToManyPersistent> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        TestInverseOneToManyPersistent.routeName,
        _bean['_title'],
        Visibility(
  ...
  SkyveDataGrid(rows: _bean['invAggAssociation'], children: [
  ]),
]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('test', 'InverseOneToManyPersistent', bizId);
      setState(() {
        _bean = bean;
      });
    }
