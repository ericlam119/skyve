...
class KitchensinkContainerGrid extends StatefulWidget {
  static const routeName = '/kitchensink/ContainerGrid';

  const KitchensinkContainerGrid({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _KitchensinkContainerGridState();
  }
}

class _KitchensinkContainerGridState extends State<KitchensinkContainerGrid> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        KitchensinkContainerGrid.routeName,
        _bean['_title'],
        Visibility(
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('URL')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'URL'), initialValue: nvl(_bean['url'])),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('kitchensink', 'ContainerGrid', bizId);
      setState(() {
        _bean = bean;
      });
    }
