...
class KitchensinkOrderedGrid extends StatefulWidget {
  static const routeName = '/kitchensink/OrderedGrid';

  const KitchensinkOrderedGrid({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _KitchensinkOrderedGridState();
  }
}

class _KitchensinkOrderedGridState extends State<KitchensinkOrderedGrid> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        KitchensinkOrderedGrid.routeName,
        _bean['_title'],
        Visibility(
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('kitchensink', 'OrderedGrid', bizId);
      setState(() {
        _bean = bean;
      });
    }
