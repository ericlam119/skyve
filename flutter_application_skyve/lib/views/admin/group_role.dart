...
class AdminGroupRole extends StatefulWidget {
  static const routeName = '/admin/GroupRole';

  const AdminGroupRole({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminGroupRoleState();
  }
}

class _AdminGroupRoleState extends State<AdminGroupRole> {
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Role Name')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('Combo ${_bean["roleName"]}'),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'GroupRole', bizId);
      setState(() {
        _bean = bean;
      });
    }
