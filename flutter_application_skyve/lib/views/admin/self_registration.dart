...
class AdminSelfRegistration extends StatefulWidget {
  static const routeName = '/admin/SelfRegistration';

  const AdminSelfRegistration({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminSelfRegistrationState();
  }
}

class _AdminSelfRegistrationState extends State<AdminSelfRegistration> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        AdminSelfRegistration.routeName,
        _bean['_title'],
        Visibility(
  ...
  SkyveVBox(children: [
    ...
    SkyveBorder(title: 'User Details', children: [
      SkyveForm(children: [
        BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
          ...
          BootstrapCol(sizes: 'col-12 col-md-8', child: 
            TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Name'), initialValue: nvl(_bean['user_contact_name'])),
          ),
        ]),
        BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
          BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Email')),
          BootstrapCol(sizes: 'col-12 col-md-8', child: 
            TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Email'), initialValue: nvl(_bean['user_contact_email1'])),
          ),
        ]),
        BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        ...
        BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
          ...
          BootstrapCol(sizes: 'col-12 col-md-8', child: 
            const SkyveButton(name: 'Register', label: 'Register'),
          ),
        ]),
      ]),
    ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'SelfRegistration', bizId);
      setState(() {
        _bean = bean;
      });
    }
