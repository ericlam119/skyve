...
class AdminUserProxy extends StatefulWidget {
  static const routeName = '/admin/UserProxy';

  const AdminUserProxy({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminUserProxyState();
  }
}

class _AdminUserProxyState extends State<AdminUserProxy> {
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      ...
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Created')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Created'), initialValue: nvl(_bean['createdDateTime'])),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'UserProxy', bizId);
      setState(() {
        _bean = bean;
      });
    }
