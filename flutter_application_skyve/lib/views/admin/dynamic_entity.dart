...
class AdminDynamicEntity extends StatefulWidget {
  static const routeName = '/admin/DynamicEntity';

  const AdminDynamicEntity({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminDynamicEntityState();
  }
}

class _AdminDynamicEntityState extends State<AdminDynamicEntity> {
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      ...
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Fields')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('TextArea ${_bean["fields"]}'),
        ),
      ]),
    ]),
  ]),
  SkyveDataGrid(rows: _bean['relations'], children: [
  ]),
]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'DynamicEntity', bizId);
      setState(() {
        _bean = bean;
      });
    }
