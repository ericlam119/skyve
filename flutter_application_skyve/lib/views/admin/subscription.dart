...
class AdminSubscription extends StatefulWidget {
  static const routeName = '/admin/Subscription';

  const AdminSubscription({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminSubscriptionState();
  }
}
...
class _AdminSubscriptionState extends State<AdminSubscription> {
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Communication')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('LookupDescription ${_bean["communication"]}'),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Receiver')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Receiver'), initialValue: nvl(_bean['receiverIdentifier'])),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Declined')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('CheckBox ${_bean["declined"]}'),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Redirect to')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Redirect to'), initialValue: nvl(_bean['preferredReceiverIdentifier'])),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'Subscription', bizId);
      setState(() {
        _bean = bean;
      });
    }
