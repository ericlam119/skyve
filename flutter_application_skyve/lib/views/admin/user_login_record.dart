...
class AdminUserLoginRecord extends StatefulWidget {
  static const routeName = '/admin/UserLoginRecord';

  const AdminUserLoginRecord({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminUserLoginRecordState();
  }
}

class _AdminUserLoginRecordState extends State<AdminUserLoginRecord> {
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      ...
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Sign In Date/Time')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Sign In Date/Time'), initialValue: nvl(_bean['loginDateTime'])),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Failed')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('CheckBox ${_bean["failed"]}'),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'UserLoginRecord', bizId);
      setState(() {
        _bean = bean;
      });
    }
