...
class AdminUserCandidateContact extends StatefulWidget {
  static const routeName = '/admin/UserCandidateContact';

  const AdminUserCandidateContact({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminUserCandidateContactState();
  }
}

class _AdminUserCandidateContactState extends State<AdminUserCandidateContact> {
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        AdminUserCandidateContact.routeName,
        _bean['_title'],
        Visibility(
  ...
  SkyveToolbar(children: [
    Container(padding: const EdgeInsets.symmetric(horizontal: 10.0), child: Wrap(alignment: WrapAlignment.center, spacing: 8.0, children: [
      ...
      const SkyveButton(name: 'null', label: 'Select'),
    ])),
  ]),
  SkyveBorder(children: [
    SkyveForm(children: [
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('admin.userCandidateContact.assocation.contact.displayName')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('LookupDescription ${_bean["contact"]}'),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Score')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Score'), initialValue: nvl(_bean['matchScore'])),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'UserCandidateContact', bizId);
      setState(() {
        _bean = bean;
      });
    }
