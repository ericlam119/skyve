...
class AdminReportDataset extends StatefulWidget {
  static const routeName = '/admin/ReportDataset';

  const AdminReportDataset({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminReportDatasetState();
  }
}

class _AdminReportDatasetState extends State<AdminReportDataset> {
  ...
  SkyveToolbar(children: [
    Container(padding: const EdgeInsets.symmetric(horizontal: 10.0), child: Wrap(alignment: WrapAlignment.center, spacing: 8.0, children: [
      ...
      const SkyveButton(name: 'null', label: 'Test Query'),
    ])),
  ]),
    ...
    SkyveVBox(children: [
      SkyveForm(children: [
        BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
          BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Dataset Name')),
          BootstrapCol(sizes: 'col-12 col-md-8', child: 
            TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Dataset Name'), initialValue: nvl(_bean['datasetName'])),
          ),
        ]),
        BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
          BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Dataset Type')),
          BootstrapCol(sizes: 'col-12 col-md-8', child: 
            Text('Radio ${_bean["datasetType"]}'),
          ),
        ]),
        BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
          BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Value')),
          BootstrapCol(sizes: 'col-12 col-md-8', child: 
            TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Value'), initialValue: nvl(_bean['query'])),
          ),
        ]),
      ]),
      SkyveForm(children: [
      ...
      SkyveForm(children: [
        ...
        BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
          BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Query Results')),
          BootstrapCol(sizes: 'col-12 col-md-8', child: 
  ...
  SkyveDataGrid(rows: _bean['parent_parameters'], children: [
  ]),
]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'ReportDataset', bizId);
      setState(() {
        _bean = bean;
      });
    }
