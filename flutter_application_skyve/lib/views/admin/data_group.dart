...
class AdminDataGroup extends StatefulWidget {
  static const routeName = '/admin/DataGroup';

  const AdminDataGroup({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminDataGroupState();
  }
}

class _AdminDataGroupState extends State<AdminDataGroup> {
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'DataGroup', bizId);
      setState(() {
        _bean = bean;
      });
    }
