...
class AdminDownloadFolder extends StatefulWidget {
  static const routeName = '/admin/DownloadFolder';

  const AdminDownloadFolder({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminDownloadFolderState();
  }
}

class _AdminDownloadFolderState extends State<AdminDownloadFolder> {
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'DownloadFolder', bizId);
      setState(() {
        _bean = bean;
      });
    }
