...
class AdminDocumentNumber extends StatefulWidget {
  static const routeName = '/admin/DocumentNumber';

  const AdminDocumentNumber({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminDocumentNumberState();
  }
}

class _AdminDocumentNumberState extends State<AdminDocumentNumber> {
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      ...
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Sequence')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Sequence'), initialValue: nvl(_bean['sequenceName'])),
        ),
      ]),
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Number')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          TextFormField(decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Number'), initialValue: nvl(_bean['documentNumber'])),
        ),
      ]),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'DocumentNumber', bizId);
      setState(() {
        _bean = bean;
      });
    }
