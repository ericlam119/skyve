...
class AdminCommunicationTemplate extends StatefulWidget {
  static const routeName = '/admin/CommunicationTemplate';

  const AdminCommunicationTemplate({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminCommunicationTemplateState();
  }
}

class _AdminCommunicationTemplateState extends State<AdminCommunicationTemplate> {
  ...
  SkyveBorder(children: [
    SkyveForm(children: [
      ...
      BootstrapRow(decoration: BoxDecoration(border: Border.all(color: Colors.transparent, width: 10.0)), children: [
        BootstrapCol(sizes: 'col-12 col-md-4', invisibleForSizes: 'xs sm', child: const SkyveLabel('Template')),
        BootstrapCol(sizes: 'col-12 col-md-8', child: 
          Text('HTML ${_bean["template"]}'),
        ),
      ]),
    ]),
  ]),
  SkyveBorder(title: 'Communications using this template', children: [
    SkyveVBox(children: [
      const Text('ListGrid'),
    ]),
  ]),
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'CommunicationTemplate', bizId);
      setState(() {
        _bean = bean;
      });
    }
