...
class AdminUserRole extends StatefulWidget {
  static const routeName = '/admin/UserRole';

  const AdminUserRole({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminUserRoleState();
  }
}

class _AdminUserRoleState extends State<AdminUserRole> {
  ...
  void _load(String? bizId) async {
    if (_bean['bizId'] == null) {
      final bean = await SkyveRestClient().edit('admin', 'UserRole', bizId);
      setState(() {
        _bean = bean;
      });
    }
