...
class AdminqReportTemplatesList extends StatefulWidget {
  static const routeName = '/admin/qReportTemplatesList';

  const AdminqReportTemplatesList({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _AdminqReportTemplatesListState();
  }
}

class _AdminqReportTemplatesListState extends State<AdminqReportTemplatesList> {
  bool _loaded = false;
  List<dynamic> _rows = [];
  ...
  Widget build(BuildContext context) {
    ...
    return SkyveView.responsiveView(
        context,
        AdminqReportTemplatesList.routeName,
  ...
  void _load() async {
    final rows = await SkyveRestClient().query('admin', 'qReportTemplates', 0, 75);
    setState(() {
      _rows = rows;
      _loaded = true;
    });
